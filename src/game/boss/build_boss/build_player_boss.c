...
void build_player_boss(game_t *game, boss_t *boss_s, int i)
{
    boss_s->background[i] = malloc(sizeof(sprite_t));
    boss_s->background[i]->scale = (sfVector2f) {1, 1};
    boss_s->background[i]->pos = (sfVector2f) {469, 822};
    boss_s->background[i]->sprite = sfSprite_create();
    boss_s->background[i]->texture =
    sfTexture_createFromFile(game->player->obj->sprite, NULL);
