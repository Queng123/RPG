...
void draw_sprite_frame_three(game_t *game)
{
    sfRenderWindow_drawSprite(game->game_info->window,
    game->intro_s->background[7]->sprite, NULL);
    sfRenderWindow_drawSprite(game->game_info->window,
...
void draw_bubulle_legende_two(game_t *game)
{
    sfRenderWindow_drawSprite(game->game_info->window,
    game->intro_s->background[7]->sprite, NULL);
    sfRenderWindow_drawSprite(game->game_info->window,
    game->intro_s->background[8]->sprite, NULL);
    sfRenderWindow_drawSprite(game->game_info->window,
...
void draw_frame_three(game_t *game)
{
    ...
    if (game->intro_s->frame_one == 3) {
        draw_sprite_frame_three(game);
        if (check_is_click_ok(game, (sfVector2f){1707, 959},
        (sfVector2f){1854, 991}, pos_mouse) == 1)
            game->intro_s->bubulle_legende = 2;
        if (game->intro_s->bubulle_legende == 2) {
            draw_bubulle_legende_two(game);
        }
        if (game->intro_s->bubulle_legende == 2 &&
        check_is_click_ok(game, (sfVector2f){39, 957},
        (sfVector2f){227, 1005}, pos_mouse) == 1)
            game->intro_s->frame_one = 4;
    }
}
